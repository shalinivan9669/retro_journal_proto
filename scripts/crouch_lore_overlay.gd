...
@export var max_font_size: int = 78
@export var min_font_size: int = 38

@onready var label: Label = $TextMargin/LoreLabel
...
func _ready() -> void:
	...
	add_to_group("crouch_lore_overlay")
	visible = false
...
func _resize_text() -> void:
	...
	var max_line_chars := 48.0
	var target_width := viewport_size.x * 0.88
	var font_size := int(floor(target_width / (max_line_chars * 0.5)))
